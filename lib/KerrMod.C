...
void KerrMod::gmunu(double g[4][4], const double * pos) const {
  double r = pos[1];
  double sth2, cth2;
...
double KerrMod::gmunu(const double * pos, int mu, int nu) const {
  double r = pos[1];
  double sth2, cth2;
  sincos(pos[2], &sth2, &cth2);
  sth2*=sth2; cth2*=cth2;
...
void KerrMod::gmunu_up(double gup[4][4], const double * pos) const {
  double r = pos[1];
  double sth2, cth2;
  sincos(pos[2], &sth2, &cth2);
  sth2*=sth2; cth2*=cth2;
...
double KerrMod::gmunu_up(const double * pos, int mu, int nu) const {
  double r = pos[1];
  double sth2, cth2;
  sincos(pos[2], &sth2, &cth2);
  sth2*=sth2; cth2*=cth2;
...
int KerrMod::christoffel(double dst[4][4][4], double const pos[4]) const
{
  int a, mu, nu;
  for (a=0; a<4; ++a)
    for(mu=0; mu<4; ++mu)
...
// Optimized version
double KerrMod::ScalarProd(const double* pos,
...
void KerrMod::computeNBeta(const double coord[4],
...
int KerrMod::diff31(const state_t &x,
		   state_t &dxdt,
		   double /* mass */) const {
  // Expressions from Eric's BH notes (N,beta,gamma)
  // and KerrMod Sage notebook (Kij)
...
/*For integration of KerrMod geodesics.
diff is such that : y_dot=diff(y,cst) where cst are constants of motion (mu,E,L,Q in KerrMod)
...
*/
int KerrMod::diff(const double* coordGen, const double* cst, double* res) const{
  double a=spin_;
  ...
  if (r < 0.) {
    cerr << "r= " << r << endl;
    GYOTO_ERROR( "KerrMod.C : r negative!!!!! the horizon may have been crossed..." );
    
  }

  if (r < rsink_) {
#   if GYOTO_DEBUG_ENABLED
    GYOTO_DEBUG << "Too close to horizon in KerrMod::diff at r= " << r << endl;
#   endif
    return 1;
  }
  ...
  if (Sigma==0) GYOTO_ERROR("In KerrMod::diff(): Sigma==0");
  ...
  if (tmp1==0)  GYOTO_ERROR("In KerrMod::diff(): 2.*Delta*Sigma==0");
  double tmp1m1=1./tmp1;

  if (Delta==0) GYOTO_ERROR("In KerrMod::diff(): Delta==0");
...
void KerrMod::circularVelocity(double const coor[4], double vel[4],
			      double dir) const {
  if (keplerian_) {
...
void KerrMod::zamoVelocity(double const * coor, double* vel) const {
  double g[4][4];
  gmunu(g, coor);
  double gtt = g[0][0],
    //    grr      = g[1][1],
...
int KerrMod::myrk4(Worldline * line, state_t const &coordin,
...
int KerrMod::myrk4(const double coor[8], const double cst[5],
		  double h, double res[8]) const{
  //  cout << "In KerrMod::myrk4 - 2" << endl;
  /*
...
int KerrMod::myrk4_adaptive(Worldline * line, state_t const &coordin,
			   ...
			   double h0, double& h1, double h1max) const
{
  ...
  while (1){
    ...
    */
	...
	if (h0/hinit>steptol){
	  GYOTO_SEVERE << "In KerrMod::myrk4_adaptive: "
    ...
    if (countbis > countbislim && zaxis) {
      ...
      GYOTO_INFO << "WARNING: " << endl
		 << "in KerrMod.C couldn't solve z-axis problem ; stopping..."
		 << endl;
      return 1;
    }
...
int KerrMod::CheckCons(const double coor_init[8], const double cst[5], double coor_fin[8]) const {
  /*
...
# endif
  if (fabs(Qtest-QQ)*QQm1 > 1e-6){//Then change thetadot to allow Qtest=QQ
	...
	if (fabs(argsqrt)>limargbis)
	  GYOTO_ERROR("In KerrMod::CheckCons Impossible to determine thetadot; "
		     "maybe try to increase parameter limarg");
	GYOTO_INFO << "KerrMod::CheckCons argsqrt= " << argsqrt << " at theta= " << coor_init[2] << ". Putting it to 0..." << endl;
	argsqrt=0.;
      }
    }
...
void KerrMod::Normalize4v(double coord[8], const double part_mass) const {
  /*
  ...
  if (part_mass==0.){// *** ZERO - NORM CASE ***
	...
	if (verbose() >= GYOTO_WARNING_VERBOSITY) {
	  GYOTO_WARNING << "0-NORM CLOSE TO HORIZON : "
               << "in KerrMod::Normalize4v impossible to normalize 0-mass "
	...
	GYOTO_ERROR( "In KerrMod::Normalize4v impossible to normalize 0-mass particle outside horizon!" );
      }	  
    }
	...
	if (verbose()>=GYOTO_WARNING_VERBOSITY) {
	  cerr << "WARNING -1 - NORM CLOSE TO HORIZON : "
	       << "in KerrMod::Normalize4v impossible to normalize massive "
	...
	GYOTO_ERROR( "In KerrMod::Normalize4v impossible to normalize massive particule outside horizon!" );
      }
    }
    ...
    GYOTO_ERROR("In KerrMod::Normalize4v: negative mass!");
  }

}


void KerrMod::MakeCoord(const double coordin[8], const double cst[5], double coord[8]) const {
...
void KerrMod::MakeMomentum(const double coord[8], const double cst[5], double coordout[8]) const{
  double EE=cst[1], LL=cst[2];
...
void KerrMod::nullifyCoord(double coord[8]) const {
  double tdot2;
  nullifyCoord(coord,tdot2);
}

void KerrMod::nullifyCoord(double coord[4], double & tdot2) const {
...
void KerrMod::computeCst(const double coord[8], double cst[5]) const{ 
...
void KerrMod::setParticleProperties(Worldline * line, const double* coord) const
{
  double cst[5];
...
void KerrMod::observerTetrad(double const pos[4], double fourvel[4],
